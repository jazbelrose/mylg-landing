...
const TheGoldPrincess = () => {
     ...
     const pageTitle = "The Gold Princess - Royal Allure Photography & Styling";
     const pageDescription = "Explore 'The Gold Princess,' a royal allure-inspired photography shoot for a perfume brand. Styled for elegance and grace, captured in pristine gold tones.";
     const canonicalUrl = "https://mylg.studio/works/The-gold-princess";


  const galleryRefs = useRef([]);
  const imageUrls = Array.isArray(goldPrincessData) ? goldPrincessData : [];
  ...
  useEffect(() => {
    if (!isLoading && svgReady) {
      ...
      const refsSnapshot = galleryRefs.current.slice();

      refsSnapshot.forEach((galleryItem) => {
        if (galleryItem) {
...
<Helmet>
      ...
      <div className={`${opacityClass} ${isModalOpen ? 'no-scroll' : ''}`}>
        ...
        <div className="workpage-heading">
          <InlineSVG src="https://d2qb21tb4meex0.cloudfront.net/svg/goldprincessheader.svg" onReady={() => setSvgReady(true)} />
        </div>
